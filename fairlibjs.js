...
* @param {string} [method=rake] Method to extract the relevant keywords, the accepted values are rake or gpt. Default is rake (free)
* @param {boolean} evaluate Flag that indicates whether if must evaluate the matchings of the predicted keywords, topics and operations to the ones previously assigned. In case the value is true, there must be the properties keywords, topics and operations in each object from the list described above, with a string array as their values.
*
...
* 
* @example
* let v = await fairlibjs.performKeyExtranctionEdamMappingInBatch( location.href+'list_workflowhub.json', 'rake', true )
*/
fairlibjs.performKeyExtranctionEdamMappingInBatch = async function(url, method = "rake", evaluate = false){
    method = method.toLowerCase()
    
    let result = { data: [] }
    let res = []
    if( ["gpt", "rake"].includes(method) ){
        ...
        let alls = performance.now()
        for( let doc of dat ){
            let desc = removeTags(doc.description)
            desc = removeMd(desc)
            desc = desc.replaceAll('\\r','').replaceAll('\\t','').replaceAll('\\n',' ')
            
            let keyst = performance.now()
            let keys = []
            if( method=="rake" ){
                keys = fairlibjs.get_rake_keywords(obj, desc, 3)
            }
            if( method=="gpt" ){
                keys = await fairlibjs.get_gpt_keywords(desc)
            }
            doc.keywords_get_time = performance.now() - keyst
            
            keys = keys.answer
            obj.keywords = keys
            let simst = performance.now()
            let sims = fairlibjs.mapEdamWithKeywords(obj)
            doc.edam_mapping_time = performance.now() - simst
            
            let flag = !evaluate
            if( evaluate ){
                if( doc.keywords && keys.length > 0 ){
                    let keyevst = performance.now()
                    doc.eval_keywords = fairlibjs.evaluateAnnotation(keys, doc.keywords, cutoff)
                    doc.eval_keywords_time = performance.now() - keyevst
                    flag = true
                }
                if( doc.topics && sims.topic.length > 0 ){
                    let tpevst = performance.now()
                    doc.eval_topics = fairlibjs.evaluateAnnotation( sims.topic.map( v => v.edamLabel ), doc.topics.map( v => v.name ), cutoff)
                    doc.eval_topics_time = performance.now() - tpevst
                    flag = true
                }
                if( doc.operations && sims.operation.length > 0 ){
                    let opevst = performance.now()
                    doc.eval_operations = fairlibjs.evaluateAnnotation( sims.operation.map( v => v.edamLabel ), doc.operations.map( v => v.name ), cutoff)
                    doc.eval_operations_time = performance.now() - opevst
                    flag = true
                }
            }
            
            doc.predicted_keywords = keys
            doc.predicted_topics = sims.topic
            doc.predicted_operations = sims.operation
            
            delete doc.description
            doc.clean_description = desc
        ...
        result.total_time = performance.now() - alls
        result.data = res
    }
    else{
        console.log("Error: Invalid method for keyword extraction (Options: rake or gpt)")
    }
...
fairlibjs.get_gpt_keywords = async (q) => {
    ...
    if(key!='' && key!=null && key!="none"){
        ...
        try{
            obj={ 'messages': messages, 'question': q }
...
* var tagA = await fairlibjs.saveFile( content, 'graph.json')
*/
